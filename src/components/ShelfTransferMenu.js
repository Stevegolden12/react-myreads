...
class ShelfTransferMenu extends Component {
  ...
  render() {
    ...
    return (
      < form >
        <select className="main-page__select" size="5" visiblity={this.props.isSelectVisible === true ? 'visible' : 'hidden'} value={this.props.shelf} onChange={(event) => { this.props.changeBookShelf(event, this.props.book.id); this.props.toggleOptionNoneSelect() }} >
